...
create_user_and_setup() {
    ...
    if id "openbao" &>/dev/null; then
        ...
        useradd --system --home /var/lib/openbao --shell /bin/bash --user-group openbao
    fi

    # Check if /var/lib/openbao directory exists, if not, create it
    if [ ! -d /var/lib/openbao ]; then
        mkdir -p /var/lib/openbao
        chown openbao:openbao /var/lib/openbao
    fi
...
install_dependencies() {
    ...
    mkdir -p /var/lib/openbao/.nvm
    chown -R openbao:openbao /var/lib/openbao
    apt install -y git make curl gnupg2
}
...
clone_and_build_openbao() {
    ...
    mkdir -p /var/lib/openbao/src/github.com/openbao
    chown -R openbao:openbao /var/lib/openbao/src/github.com
    cd /var/lib/openbao/src/github.com/openbao
    if [ ! -d "openbao" ]; then
        git clone https://github.com/openbao/openbao.git
    fi
    cd openbao
    chown -R openbao:openbao /var/lib/openbao/src/github.com/openbao/openbao
    ...
    if [ -f /var/lib/openbao/src/github.com/openbao/openbao/bin/bao ]; then
        mv /var/lib/openbao/src/github.com/openbao/openbao/bin/bao /usr/local/bin/openbao
    else
...
configure_openbao() {
    ...
    mkdir -p /var/lib/openbao/config
    cat << 'EOF' | tee /var/lib/openbao/config/config.hcl
ui = true
...
EOF
    ...
    chmod -R 755 /var/lib/openbao/data
}

create_openssl_config() {
    # Step 11: Create OpenSSL Configuration File
    cat << 'EOF' | tee /var/lib/openbao/openssl.cnf
[req]
default_bits       = 2048
default_md         = sha256
prompt             = no
...
generate_private_key_and_certificate() {
    ...
    openssl genpkey -algorithm RSA -out /var/lib/openbao/tls.key
    openssl req -new -x509 -days 365 -key /var/lib/openbao/tls.key -out /var/lib/openbao/tls.crt -config /var/lib/openbao/openssl.cnf

    mkdir -p /var/lib/openbao/tls
    mv /var/lib/openbao/tls.crt /var/lib/openbao/tls/
    mv /var/lib/openbao/tls.key /var/lib/openbao/tls/
    chown -R openbao:openbao /var/lib/openbao/tls/

    cp /var/lib/openbao/tls/tls.crt /usr/local/share/ca-certificates/openbao.crt
...
initialize_and_unseal_openbao() {
    ...
    su -u openbao -c 'nohup openbao server -config /var/lib/openbao/config/config.hcl > /var/lib/openbao/openbao.log 2>&1 &'
    ...
    if ! curl -sk https://<IP address or URL>:8200/v1/sys/seal-status; then
        ...
        su -u openbao -c 'cat /var/lib/openbao/openbao.log'
        exit 1
    fi

    echo "Execute command: openbao operator init"

    su -u openbao -c 'export VAULT_ADDR="https://<IP address or URL>:8200" && openbao operator init > /tmp/init_output.txt 2>&1'
...
create_unseal_script() {
    # Step 16: Create a script to unseal OpenBao
...
create_env_file() {
    # Step 17: Create environment file for OpenBao
    mkdir -p /etc/openbao.d
    cat << 'EOF' | tee /etc/openbao.d/openbao.env
...
create_systemd_services() {
    # Step 18: Create openbao.service for systemd
    cat << 'EOF' | tee /etc/systemd/system/openbao.service
[Unit]
Description=OpenBao
...
EOF
    ...
    # Create openbao-unseal.service for systemd
    cat << 'EOF' | tee /etc/systemd/system/openbao-unseal.service
[Unit]
...
kill_openbao() {
    # Step 19: Gracefully stop the openbao server
...
system_services() {
    # Step 20: Reload systemd and enable services
